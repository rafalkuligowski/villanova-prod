...
class Help
{
    ...
    const DOCS_EXPIRE_OPT_KEY = 'duplicator_pro_help_docs_expire';
    ...
    private function getDataFromApi(): array
    {
        ...
        if ($categories === [] || $articles === [] || $tags === []) {
            DUP_PRO_Log::trace('Failed to load from API. No data.');
            return [];
        }
     ...
     */
    private function fetchDataFromEndpoint($endpoint, $limit, $fields = []): array
    {
        $result      = [];
        ...
        for ($i = 1; $i <= $maxPages; $i++) {
            ...
            if (is_wp_error($response)) {
                DUP_PRO_Log::trace("Failed to load from API: {$endpointUrl}");
                DUP_PRO_Log::trace($response->get_error_message());
                return [];
            }

            $code = wp_remote_retrieve_response_code($response);
            if ($code !== 200) {
                DUP_PRO_Log::trace("Failed to load from API: {$endpointUrl}, code: {$code}");
                return [];
            }

            $body = wp_remote_retrieve_body($response);
            if (($data = json_decode($body, true)) === null) {
                DUP_PRO_Log::trace("Failed to decode response: {$body}");
                return [];
            }
    ...
    private static function getCurrentPageTag(): string
    {
        ...
        switch ($page) {
            ...
            case ControllersManager::SETTINGS_SUBMENU_SLUG:
                return 'settings';
            default:
                DUP_PRO_Log::trace("No tag for page.");
        }
    ...
    {
        return DUPLICATOR_PRO_SSDIR_PATH . '/cache_' . md5(CryptBlowfish::getDefaultKey()) . '/duplicator_pro_help_cache.json';
    }
    ...
    private function setFromArray($data): bool
    {
        if (!isset($data['articles']) || !isset($data['categories']) || !isset($data['tags'])) {
            DUP_PRO_Log::trace("Invalid data.");
            return false;
        }
    ...
    private function loadData()
    {
        if (!file_exists(self::getCacheFilePath())) {
            DUP_PRO_Log::trace("Cache file does not exist: " . self::getCacheFilePath());
            return false;
        }

        if (($contents = file_get_contents(self::getCacheFilePath())) === false) {
            DUP_PRO_Log::trace("Failed to read cache file: " . self::getCacheFilePath());
            return false;
        }

        if (($data = json_decode($contents, true)) === null) {
            DUP_PRO_Log::trace("Failed to decode cache file: " . self::getCacheFilePath());
            return false;
        }
    ...
    public function updateData()
    {
        if (($data = $this->getDataFromApi()) === []) {
            DUP_PRO_Log::trace("Failed to load data from API.");
            return false;
        }
        ...
        if (!file_exists($cacheDir) && !SnapIO::mkdir($cacheDir, 0755, true)) {
            DUP_PRO_Log::trace("Failed to create cache directory: {$cacheDir}");
            return false;
        }

        if (($encoded = SnapJson::jsonEncode($data)) === false) {
            DUP_PRO_Log::trace("Failed to encode cache data.");
            return false;
        }

        if (file_put_contents(self::getCacheFilePath(), $encoded) === false) {
            DUP_PRO_Log::trace("Failed to write cache file: {$cachePath}");
            return false;
        }
