...
class ScanIterator implements GenericSeekableIteratorInterface
{
     ...
     * @return string[]
     */
    protected function getNodeChildsPaths(ScanNodeInfo $node)
    {
        if (!$node->isDir()) {
            return [];
        }

        if (!SnapOS::isWindows() && !$node->isReadable()) {
            return [];
        }
        ...
        if (($childs = scandir($node->getPath(), $sort)) == false) {
            return [];
        }

        $childs = array_filter($childs, fn($child): bool => $child !== '.' && $child !== '..');
