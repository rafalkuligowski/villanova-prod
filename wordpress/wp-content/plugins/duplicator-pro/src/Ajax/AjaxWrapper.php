...
class AjaxWrapper
{
    ...
    public static function json(
        ...
        try {
            ...
            DUP_PRO_Log::traceException($e, 'Error executing ajax callback');
            $error             = true;
            $result['message'] = $e->getMessage();
        } finally {
            $result['output'] = ob_get_clean();
            if ($errorUnespectedOutput && !empty($result['output'])) {
                DUP_PRO_Log::trace('Unexpected output: ' . substr($result['output'], 0, 250));
                $error = true;
            }
        }
    ...
    public static function fileDownload(
        ...
        try {
            ...
            DUP_PRO_Log::traceException($e, 'Error executing ajax callback file download');
