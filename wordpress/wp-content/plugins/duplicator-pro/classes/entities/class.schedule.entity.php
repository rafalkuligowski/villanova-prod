...
class DUP_PRO_Schedule_Entity extends AbstractEntityList implements UpdateFromInputInterface, ModelMigrateSettingsInterface
{
    ...
    public function process()
    {
        ...
        if ($this->active && ($this->next_run_time <= $now)) {
            ...
            try {
                ...
                DUP_PRO_Log::trace("NEXT RUN IS NOW! $next_run_time_string <= $now_string so trying to queue Backup");
    ...
    public function insert_new_package($run_now = false)
    {
        ...
        DUP_PRO_Log::trace("Archive build mode before calling insert new Backup, build mode:" . $global->getBuildMode());
        ...
        DUP_PRO_Log::trace('NEW PACKAGE NAME ' . $package->getName());
        ...
        DUP_PRO_Log::trace("archive build mode after calling insert new Backup ID = " . $package->ID . " build mode = " . $global->archive_build_mode);
    }
     ...
     * @return self[]
     */
    public static function get_by_template_id($template_id): array
    {
