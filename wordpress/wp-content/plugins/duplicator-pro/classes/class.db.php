...
class DUP_PRO_DB extends wpdb
{
     ...
     */
    public static function getTableCharSetList($tables): array
    {
        $charSets = [];
        try {
    ...
    public static function getTableCollationList($tablesToInclude)
    {
        ...
        if (is_null($collations)) {
            ...
            foreach (array_chunk($tablesToInclude, self::MAX_TABLE_COUNT_IN_PACKET) as $tablesChunk) {
                $sqlTables = implode(",", array_map([self::class, 'escValueToQueryString'], $tablesChunk));
        ...
        if (is_null($engines)) {
            ...
            foreach (array_chunk($tablesToInclude, self::MAX_TABLE_COUNT_IN_PACKET) as $tablesChunk) {
                $query = "SELECT DISTINCT `ENGINE` FROM `information_schema`.`tables` WHERE `ENGINE` IS NOT NULL AND `table_schema` = '{$wpdb->dbname}' "
                    . "AND `table_name` in (" . implode(",", array_map([self::class, 'escValueToQueryString'], $tablesChunk)) . ")";
     ...
     * @return int
     */
    public static function getImpreciseTotaTablesRows($tables = [])
    {
        $tables = (array) $tables;
        if (count($tables) == 0) {
            return 0;
        ...
        $query  = 'SELECT SUM(TABLE_ROWS) as "totalRows" FROM information_schema.TABLES '
            ...
            . 'AND TABLE_NAME IN (' . implode(',', array_map([self::class, 'escValueToQueryString'], $tables)) . ')';
